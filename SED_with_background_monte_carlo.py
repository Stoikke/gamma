...
if os.path.isdir(fond_dir):
    ...
    for fichier in fond_files:
        ...
        dE    = emax - emin
        emean = (emin+emax)/2 #np.sqrt(emin * emax)
...
for fichier in fichiers:
    ...
    flux     = counts[mask] / exposure[mask]
    flux_unc = errors[mask] / exposure[mask]
    # print("flux", flux, "flux_unc", flux_unc)
    N         = len(flux)
    flux_mean = np.mean(flux)
    flux_err  = np.sqrt(np.sum(flux_unc**2)) / N
    ...
    net_dphi_dE     = np.abs(flux_mean / dE - fond_dphi_dE) # - fond_dphi_dE
    net_dphi_dE_err = np.sqrt((flux_err / dE)**2 + fond_dphi_dE_err**2)#+ fond_dphi_dE_err**2
...
# print(energies_mean, flux_moyens, flux_erreurs, dE_half_arr)
masque_valid = (flux_moyens >= 0) & (flux_erreurs > 0) & (flux_erreurs < flux_moyens)
# print("masque_valid", masque_valid)
